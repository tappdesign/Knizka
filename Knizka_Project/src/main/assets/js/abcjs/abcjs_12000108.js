var MusicScoreData =
`
%%composerfont Tahoma 9 italic
%%infofont Tahoma 9 italic
X:1
C:M. Sch. T.
R:V. W. (1924)
L:1/4
M:4/4
I:linebreak $
K:A
 E A c A | A F A !fermata!E | %2
w: ~Hľa,~ ~a~- ~ká~ ~to|hviez~- ~da~ ~krá~- ~sna|
 E A B c |$ B F G z | %4
w: ~z ne~- ~ba~ ~sa~ ~na|nás~ ~sme~- ~je:|
 E A c A | A F A !fermata!E |$ %6
w: ~a~ ~to~ ~je~ ~tá|hviez~- ~da~ ~šťast~- ~ná,|
 A c B G | G F E2 | %8
w: ~hviez~- ~da~ ~na~- ~šej|ná~- ~de~- ~je.|
 E F G A |$ B A A !fermata!G | %10
w: ~Daj~- ~me~- ~že~ ~sa|my~ ~na~ ~ces~- ~tu,|
 G A B c | d c c !fermata!B |$ %12
w: ~k a~- ~ké~- ~mu~ ~tá|hviez~- ~da~ ~mes~- ~tu|
 E A c A | A F A E | %14
w: nás~ pri- ve- die|pre- ra- dost- ných,|
 F d c B |$ A G A2 |] %16
w: ~Kris~- ~ta~ ~vi~- ~dieť|žia~- ~dost~- ~ných.|

`;
